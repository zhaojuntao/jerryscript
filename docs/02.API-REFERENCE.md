...
它包括在相关的JavaScript对象被垃圾收集时调用的自由回调。
如果不需要，它可以是空的。
...
这样，每个 `const jerry_object_native_info_t *` 指针地址本身唯一标识一个 C 类型的原生指针  
...
创建新的JavaScript对象，就像new Object（）一样。
**Prototype**
...
指针和它的类型信息是由 `jerry_set_object_native_pointer` 预先关联的  
...
在分配和访问 `out_native_pointer_p` 前， 建议确保 `out_native_info_p` 指针值与被期望的原生info指针值是相同的。  
这是一个很重要的例子：期望`this`具有某种C类型的本地指针的外部函数。  
可以在JavaScript中使用`call（）`，`apply()` or `bind()`来随意更改`this`。   
因此，`this`的本地指针可能不是预期的C类型。 为了安全可靠地处理这个问题，必须添加类型检查，以确保`out_native_pointer_p`具有预期的类型，然后才转换和取消引用`out_native_pointer_p`。   
...
- return value
  - true, if there is native pointer associated with the object如果有与该对象关联的本机指针  
  - false, otherwise
...
//type_info的地址（！）用作唯一“标识”C类型“native_obj_t *”的方法。
static const jerry_object_native_info_t native_obj_type_info =
{
  .free_cb = native_freecb
};

// Function creating JS object that is "backed" by a native_obj_t *: 创建由native_obj_t *“支持”的JS对象的函数：  
{
  ...
...
// Native method, `this` is expected to be "backed" by a native_obj_t *: Native方法，`this`期望被native_obj_t *“支持”： 
{
  void *native_p;
  ...
  if (has_p)
  {
    // The type_p pointer address itself is used to identify the type: type_p指针地址本身用于标识类型
    if (type_p == &native_obj_type_info)
    {
      ...
      // pointer: 只有现在转换指针为 `native_obj_t *` 和 解引用指针才是安全的  
...
为指定的对象设置原生指针和可选类型信息。
稍后您可以通过调用jerry_get_object_native_pointer来获取它们。
...
如果在原生类型信息中指定了非空的释放回调，则在释放对象时将由垃圾回收器调用。类型info始终覆盖之前的值，因此传递NULL值将删除当前的类型信息。        

#  Common external function handlers 常见的外部函数处理程序  
...
脚本断言。在失败断言处，程序调用  `jerry_port_fatal` 
...
提供脚本的 `print` 实现。
该例程将其所有参数转换为字符串，并使用 `jerryx_port_handler_print_char` 来输出它们。
NUL字符输出为“\ u0000”，其他字符输出bytewise。

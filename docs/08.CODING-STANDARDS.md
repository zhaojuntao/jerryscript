...
本文简要概述了JerryScript编码标准。
每个规则从一个简短的描述开始，然后是几个
的例子。我们相信例子比长时间的解释好。
当你提交一个补丁时，请遵循这些指导方针
复习
...
* 最大行长度为120字符(不包括换行)。
* 不允许拖尾空白。
* 运行 `tools/run-tests.py --check-vera` 自动检查几个编码约定。
...
在JerryScript中只允许块注释(' /* */ ')。
注释应该是完整的句子(例如:以大写字母开头)，除了返回值、字段和
参数描述(参见下面的例外)。在
注释的正文部分应该重点解释 “代码做了一些事情” 而不是 “代码在干什么”。
...
```c
/**
 * Short description about the constant.关于常数的简短描述。
 */
...
## Ternary conditional operators 三元条件运算符  
